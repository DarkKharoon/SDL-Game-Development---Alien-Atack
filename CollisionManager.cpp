...
void CollisionManager::checkPlayerEnemyBulletCollision(Player* pPlayer)
{
	...
	for (int i = 0; i < TheBulletHandler::Instance()->
		getEnemyBullets().size(); i++)
	{
		EnemyBullet* pEnemyBullet = TheBulletHandler::Instance()->
			getEnemyBullets()[i];
	...
	const std::vector<GameObject*>& objects)
{
	for (int i = 0; i < objects.size(); i++)
	{
		...
		for (int j = 0; j < TheBulletHandler::Instance()->
			getPlayerBullets().size(); j++)
		{
			if (pObject->type() != std::string("Enemy") || 
				!pObject->updating())
			{
				continue;
			}
		...
		collisionLayers.begin(); it != collisionLayers.end(); ++it)
	{
			...
			getVelocity().getY() >= 0)
		{
			...
			tileRow = ((pPlayer->getPosition().getY() + pPlayer ->
				getHeight()) / pTileLayer->getTileSize());
